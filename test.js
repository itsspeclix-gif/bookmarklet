javascript:(async function _main() {
    ...
    function loadLibrary(games, userName, startTime) {
        ...
        // Consume any pending write-game delta immediately on load
        ...
        document.write('#tbar{position:fixed;top:0;left:0;height:3px;background:linear-gradient(90deg,var(--ac),var(--ac2),#a78bfa);z-index:9999;border-radius:0 2px 2px 0;box-shadow:0 0 10px rgba(59,130,246,.5);transition:width 1s linear;}');
        ...
        document.write('.ibtn{background:var(--s2);border:1px solid var(--b2);color:var(--t1);width:36px;height:36px;border-radius:50%;cursor:pointer;display:grid;place-items:center;font-size:15px;transition:.2s;}.ibtn:hover{background:var(--ac);border-color:var(--ac);color:#fff;}#settingsBtn:hover{transform:rotate(35deg);}');
        ...
        document.write('#favSec,#recSec{display:none;margin-bottom:44px;text-align:left;}.slbl{font-size:.78rem;font-weight:700;text-transform:uppercase;color:var(--t2);letter-spacing:1px;margin-bottom:16px;display:flex;align-items:center;gap:6px;}#favGrid,#recGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:16px;}');
        ...
        document.write('#gOver{position:fixed;inset:0;background:#000;z-index:2000;display:none;flex-direction:column;opacity:0;transition:opacity .35s;}#gOver.vis{opacity:1;}#closeGame{position:absolute;top:18px;right:18px;z-index:2001;}#closeBtn{background:rgba(255,255,255,.1);color:#fff;border:1px solid rgba(255,255,255,.2);width:40px;height:40px;border-radius:50%;cursor:pointer;font-size:18px;display:grid;place-items:center;backdrop-filter:blur(8px);transition:.2s;padding:0;}#closeBtn:hover{background:var(--red);border-color:var(--red);}iframe{border:none;flex-grow:1;width:100%;height:100%;opacity:0;transition:opacity .5s;}iframe.ld{opacity:1;}');
        document.write('</style></head><body>');
        document.write('<div id="tbar" style="width:100%;"></div>');
        document.write('<nav><div class="nlogo">&#9881; <span>Library</span></div><div id="npBadge"><div id="npDot"></div><span id="npTxt">Now Playing</span></div><div class="nr"><button class="ibtn" id="settingsBtn" title="Profile">&#128100;</button><div class="umenu"><div class="uinfo" id="toggleM"><span id="uName"></span><div id="pfpCircle"></div></div><div class="dd" id="drp"><div class="di" id="pdMi">&#128100;&nbsp; Profile</div><div class="di" id="avMi">&#128247;&nbsp; Change Avatar</div><input type="file" id="pfpInp" style="display:none" accept="image/*"><hr class="ddiv"><div class="di"><div class="tw"><svg viewBox="0 0 26 26" style="width:26px;height:26px;flex-shrink:0;transform:rotate(-90deg);"><circle cx="13" cy="13" r="11" fill="none" stroke="var(--b2)" stroke-width="3"/><circle id="trFg" cx="13" cy="13" r="11" fill="none" stroke="var(--ac)" stroke-width="3" stroke-dasharray="69.1" stroke-dashoffset="0" stroke-linecap="round" style="transition:stroke-dashoffset 1s linear,stroke .5s;"/></svg><span style="font-size:13px;">Inactive: <strong id="timer" style="font-variant-numeric:tabular-nums;">30:00</strong></span></div></div><hr class="ddiv"><div class="di dred" id="soBtn">&#10005;&nbsp; Sign Out</div></div></div></div></nav>');
        document.write('<div id="pOv"></div><div id="pPanel"><div class="ph"><button class="phClose" id="phClose">&#10005;</button><div class="phAv" id="phAv"></div><div class="phName" id="phName"></div><div class="phSub" id="phSub"></div></div><div class="ps"><div class="pst">&#127775; Appearance</div><div class="trow"><button class="tbtn" data-theme="dark">&#127769; Dark</button><button class="tbtn" data-theme="light">&#9728; Light</button></div></div><div class="ps"><div class="pst">&#128202; Stats</div><div class="sg" id="sg"></div></div><div class="ps"><div class="pst">&#9201; Playtime Breakdown</div><div id="ptList"></div></div><div class="ps"><div class="pst">&#10084; Favorites</div><div id="pFavs"></div></div><div class="ps"><div class="pst">&#128336; Recently Played</div><div id="pRecent"></div></div><div class="ps"><div class="pst">&#127942; Achievements</div><div id="pAch"></div></div></div>');
        ...
        document.write('<main><div class="dg"><div class="ditem"><div class="dl">Status</div><div class="dv" style="color:var(--grn)">&#9679; Online</div></div><div class="ditem"><div class="dl">Resources</div><div class="dv" id="rCount"></div></div><div class="ditem"><div class="dl">System Date</div><div class="dv" id="sDate"></div></div></div><div class="wt"><div class="wg" id="wg"></div><div class="ws" id="ws"></div></div><div class="sw"><input type="text" id="sb" class="sbar" placeholder="Search games..."><button class="sclr" id="sc">&#10005;</button></div><div id="favSec"><div class="slbl">&#10084; Favorites</div><div id="favGrid"></div></div><div id="recSec"><div class="slbl">&#128336; Recently Played</div><div id="recGrid"></div></div><div id="noRes"><div style="font-size:50px;margin-bottom:14px;">&#127918;</div><div style="font-size:15px;font-weight:600;">No games found</div><div style="font-size:13px;margin-top:6px;color:var(--t3);">Try a different search</div></div><div id="list"></div></main></body></html>');
        document.close();
        ...
        // Save iframe game time on page unload/refresh
        window.addEventListener('beforeunload',svCurTime);

        // Consume write-game time delta
        function consDelta(){
        ...
        // Poll every 2s — catches tab-close beforeunload case
        setInterval(consDelta,2000);
        // Also consume immediately when library regains focus
        window.addEventListener('focus',function(){writeGameOpen=false;resetAct();consDelta();});
        ...
        // ── INACTIVITY TIMER ──
        var lastAct=Date.now(), prevRem=sessionLimit, circ=2*Math.PI*11;
        ...
        setInterval(function(){
            ...
            if(playing||writeGameOpen)resetAct(); // freeze timer while game is active

            var rem=Math.max(0,sessionLimit-(Date.now()-lastAct));
            var pct=rem/sessionLimit;
            var m=Math.floor(rem/60000),s=Math.floor((rem%60000)/1000);

            var timerEl=document.getElementById('timer');
            if(timerEl){timerEl.innerText=m+':'+(s<10?'0':'')+s;timerEl.style.color=rem<5*60*1000?'#f87171':rem<10*60*1000?'#fbbf24':'var(--t1)';}

            var tbar=document.getElementById('tbar');
            if(tbar)tbar.style.width=(pct*100)+'%';

            var ring=document.getElementById('trFg');
            if(ring){ring.style.strokeDashoffset=(circ*(1-pct)).toFixed(2);ring.style.stroke=rem<5*60*1000?'#f87171':rem<10*60*1000?'#fbbf24':'var(--ac)';}

            if(!playing&&!writeGameOpen&&prevRem>5*60*1000&&rem<=5*60*1000)toast('5 min of inactivity left — keep playing!','&#9200;');
            prevRem=rem;
                    ...
                    if(nw){
                        nw.addEventListener('load',function(){
                            ...
                            setTimeout(function(){
                                ...
                                btn.innerHTML='\u2715';
                                // display:grid;place-items:center fixes the centering issue
        ...
        Object.keys(games).forEach(function(name,idx){
            ...
            var ic=gd.icon;
            card.innerHTML='<button class="hrt'+(isFv(name)?' on':'')+'" data-g="'+name+'">&#10084;</button><div class="ib">'+(ic?'<img src="https://drive.google.com/uc?export=view&id='+ic+'" onload="this.style.opacity=1">':'')+'</div><div class="cn">'+name+'</div>';
